...
class XrayTestService : Service() {
    ...
    init {
        // go.Seq no longer auto-loads a native library (patched out so the
        // same gomobile runtime classes can serve both libgojni in :xray and
        // libbox in :singbox). Each process loads its own core explicitly,
        // before any gomobile class is touched.
    ...
    private fun handle(msg: Message) {
        ...
        when (msg.what) {
            XrayIpc.REQ_LATENCY -> scope.launch {
                ...
                if (profile == null) {
                    ir.swiftvpn.engine.DiagnosticLog.write(
                        ir.swiftvpn.engine.DiagnosticLog.TEST,
                        "xray latency: profile $uuid not found in :xray process",
                    )
                }
                ...
                ir.swiftvpn.engine.DiagnosticLog.write(
                    ir.swiftvpn.engine.DiagnosticLog.TEST,
                    "xray latency ${profile?.name ?: uuid}: ${ms ?: -1}ms",
                )
                replyTo.sendReply(
                    XrayIpc.RES_LATENCY,
