...
object XrayTester {
    ...
    suspend fun latency(
        ...
        when (profile.protocol) {
            Protocol.XRAY -> {
                ...
                val outbound = store?.outbound(profile.uuid) ?: return@withContext null
                ...
                mutex.withLock {
                    ...
                    CrashReporter.breadcrumb(context, "latency test ${outbound.address}")
                    val ms = xrayLatency(outbound)
                    CrashReporter.breadcrumb(context, null)
                    delay(CORE_COOLDOWN_MS)
                    ms
                }
            }
            // No proxy to dial through, so time the TCP handshake instead. This
            // says "the server is up and reachable", which is the useful part of
            // a latency badge, without pretending to have measured a tunnel.
            Protocol.OPENVPN, Protocol.WIREGUARD ->
                tcpLatency(profile.server, profile.port.toIntOrNull() ?: return@withContext null)
            // IKEv2 speaks UDP/500 — a TCP probe would always fail, so skip it.
            Protocol.IKEV2 -> null
        }
    }
     ...
     */
    private fun xrayLatency(outbound: XrayOutbound): Long? = runCatching {
        val cfg = XrayProbeConfig.buildOutboundOnly(outbound)
    ...
    }.onFailure { Log.d(TAG, "xray latency failed: ${it.message}") }.getOrNull()
    ...
    suspend fun probe(context: Context, uuid: String, store: XrayStore): ProbeResult =
        withContext(Dispatchers.IO) {
            ...
            val outbound = store.outbound(uuid)
            ...
            mutex.withLock {
                ...
                try {
                    CrashReporter.breadcrumb(context, "probe ${outbound.address}")
                    controller = startProbeInstance(outbound, port)
    ...
    suspend fun downloadSpeed(
        ...
        val outbound = store.outbound(uuid) ?: return@withContext null
        if (!XrayRuntime.ensure(context)) return@withContext null
        ...
        mutex.withLock {
            ...
            var controller: CoreController? = null
            try {
                CrashReporter.breadcrumb(context, "speed test ${outbound.address}")
                controller = startProbeInstance(outbound, port) ?: return@withContext null
                val proxy = Proxy(Proxy.Type.SOCKS, InetSocketAddress("127.0.0.1", port))
     ...
     */
    private fun startProbeInstance(outbound: XrayOutbound, port: Int): CoreController? =
        runCatching {
            val cfg = XrayProbeConfig.build(outbound, port)
