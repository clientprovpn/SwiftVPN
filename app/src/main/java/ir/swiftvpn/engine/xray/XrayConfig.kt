...
object XrayConfig {
    ...
    /**
     * Merges a user-supplied FULL Xray config (the "Custom config" profile type)
     * with the pieces only we can provide: the TUN inbound reading the VPN fd,
     * the stats/policy counters the speedometer reads, and a routing rule that
     * sends tunnel traffic to the user's first outbound.
     *
     * The user's own inbounds/routing rules are kept and ours are PREPENDED, so
     * a hand-written config keeps its extras (dokodemo doors, custom routing)
     * while the TUN path always exists. Returns null when the JSON is unusable.
     */
    fun buildFromCustom(jsonText: String, mtu: Int = 1500): String? = runCatching {
        val root = JSONObject(jsonText)
        val outbounds = root.optJSONArray("outbounds")
            ?: JSONArray().also { root.put("outbounds", it) }
        if (outbounds.length() == 0) return null

        // The proxy target must be nameable for routing and stats: tag the
        // first outbound when the author left it untagged.
        val first = outbounds.getJSONObject(0)
        val proxyTag = first.optString("tag").ifBlank {
            first.put("tag", PROXY_TAG)
            PROXY_TAG
        }
        ...
        if (root.optJSONObject("stats") == null) root.put("stats", JSONObject())
        root.put(
            "policy",
            (root.optJSONObject("policy") ?: JSONObject()).put(
                "system",
                ((root.optJSONObject("policy")?.optJSONObject("system")) ?: JSONObject())
                    .put("statsOutboundUplink", true)
                    .put("statsOutboundDownlink", true)
                    .put("statsInboundUplink", true)
                    .put("statsInboundDownlink", true),
        ...
        // Our TUN inbound goes FIRST so it cannot be shadowed, and any inbound
        // of the user's that collides with the TUN (protocol tun) is dropped —
        // two TUN readers on one fd would deadlock the core.
        val inbounds = JSONArray().put(tunInbound(mtu))
        root.optJSONArray("inbounds")?.let { existing ->
            for (i in 0 until existing.length()) {
                val inbound = existing.optJSONObject(i) ?: continue
        ...
        // Prepend the tunnel rule to the user's own routing rules.
        ...
        root.optJSONObject("routing")?.optJSONArray("rules")?.let { existing ->
            for (i in 0 until existing.length()) rules.put(existing.get(i))
        }
        root.put(
            "routing",
            (root.optJSONObject("routing") ?: JSONObject())
                .put("rules", rules),
        )
    ...
    /** True when [text] is a raw JSON config rather than a share link. */
    fun isCustomJson(text: String): Boolean = text.trimStart().startsWith("{")

    /**
     * Validates a custom config for storing: parseable JSON with at least one
     * outbound. Connection-level validity is the core's job to report.
     */
    fun isValidCustomJson(text: String): Boolean = runCatching {
        val root = JSONObject(text)
        (root.optJSONArray("outbounds")?.length() ?: 0) > 0
    }.getOrDefault(false)
    ...
    private fun proxyOutbound(o: XrayOutbound): JSONObject {
        ...
        val settings = when (o.protocol) {
            ...
            "socks" -> socksHttpSettings(o, userKey = "user")
            "http" -> socksHttpSettings(o, userKey = "user")
            else -> JSONObject()
        }
    ...
    /**
     * SOCKS / HTTP outbound (xray-core supports both natively). Login is
     * optional; when present it goes into the server entry's `users` list.
     */
    private fun socksHttpSettings(o: XrayOutbound, userKey: String): JSONObject {
        val server = JSONObject()
            .put("address", o.address)
            .put("port", o.port)
        if (o.username.isNotBlank()) {
            server.put(
                "users",
                JSONArray().put(
                    JSONObject()
                        .put(userKey, o.username)
                        .put("pass", o.password),
                ),
            )
        }
