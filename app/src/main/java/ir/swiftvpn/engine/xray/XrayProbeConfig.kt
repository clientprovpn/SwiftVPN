...
object XrayProbeConfig {
    ...
    /**
     * Probe variants for "Custom config" profiles, whose outbound already IS
     * JSON. We take the user's first outbound verbatim, tag it for routing,
     * and wrap it in the same probe scaffolding as [build]/[buildOutboundOnly].
     */
    private fun customProxyArray(jsonText: String): JSONArray? = runCatching {
        val src = JSONObject(jsonText).optJSONArray("outbounds") ?: return null
        if (src.length() == 0) return null
        val first = src.getJSONObject(0)
        if (first.optString("tag").isBlank()) first.put("tag", PROXY_TAG)
        JSONArray().put(first)
            .put(JSONObject().put("protocol", "freedom").put("tag", "direct"))
    }.getOrNull()

    fun buildCustom(jsonText: String, socksPort: Int): String? {
        val outbounds = customProxyArray(jsonText) ?: return null
        return JSONObject()
            .put("log", JSONObject().put("loglevel", "none"))
            .put(
                "inbounds",
    ...
    fun buildOutboundOnlyCustom(jsonText: String): String? {
        val src = runCatching { JSONObject(jsonText).optJSONArray("outbounds") }
            .getOrNull() ?: return null
        if (src.length() == 0) return null
        return JSONObject()
            .put("log", JSONObject().put("loglevel", "none"))
            .put("outbounds", JSONArray().put(src.getJSONObject(0)))
            .toString()
    }
}
