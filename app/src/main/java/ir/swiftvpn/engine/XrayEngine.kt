...
object XrayEngine {
    ...
    fun buildConfig(context: Context, uuid: String): String? {
        ...
        val raw = store.link(uuid) ?: return null
        // A "Custom config" profile is a full JSON document: merge our TUN
        // inbound into it instead of generating a config from a link.
        if (ir.swiftvpn.engine.xray.XrayConfig.isCustomJson(raw)) {
            return ir.swiftvpn.engine.xray.XrayConfig.buildFromCustom(raw)
        }
        val outbound = store.outbound(uuid) ?: return null
