...
object ZipImport {
    ...
    /**
     * Sniffs the zip magic (PK\x03\x04 / PK\x05\x06) from the first bytes, so
     * "Import from file" catches archives even when the file manager hides or
     * mangles the extension — there is no separate zip picker anymore.
     */
    fun hasZipMagic(context: Context, uri: android.net.Uri): Boolean = runCatching {
    ...
    private fun isConfigName(base: String): Boolean {
        ...
        return lower.endsWith(".conf") ||
            lower.endsWith(".ovpn") ||
            // SwiftVPN's own backup zips store Xray share links as .link and
            // Xray custom configs as xray/*.json (IKEv2 backups are also json,
            // but only under ikev2/ — those are matched earlier by path).
            lower.endsWith(".link") ||
            lower.endsWith(".json") ||
            lower.endsWith(".txt") ||
