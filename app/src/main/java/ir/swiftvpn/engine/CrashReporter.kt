...
object CrashReporter {
    ...
    fun clear(context: Context) {
        ...
        // NOTE: the tombstone files are deliberately NOT deleted here. The
        // crash dialog tells the user "(Full tombstone attached in the
        // diagnostic export.)" — a real lie before, because dismissing the
        // dialog ran clear() and wiped the tombstone, so the export the user
        // sent right afterwards had nothing attached. The summary file drives
        // the dialog, so only it is cleared; the raw tombstone stays for the
        // next export and is overwritten/deduped by captureNativeTombstone's
        // timestamp marker anyway.
