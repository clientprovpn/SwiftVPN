...
object BackupManager {
    ...
    fun exportAll(context: Context, profiles: List<Profile>): File? {
        ...
        return runCatching {
            var written = 0
            // Entry name -> original profile name, and entry name -> OpenVPN
            // credentials. The zip entry name is name-mangled (safeName) and
            // a .ovpn file cannot carry a username/password at all, so both
            // travel in the manifest instead — the zip stays loadable by other
            // clients, and a SwiftVPN restore gets the full fidelity back.
            val origNames = mutableMapOf<String, String>()
            val creds = mutableMapOf<String, Pair<String, String>>()
            ZipOutputStream(out.outputStream().buffered()).use { zip ->
                ...
                for (p in profiles) {
                    val (dirName, ext) = when (p.protocol) {
                        ...
                        // Xray profiles are either share links (.link) or raw
                        // JSON custom configs (.json) — keep the real extension
                        // so a restore lands on the same parser path.
                        Protocol.XRAY -> "xray" to
                            if (File(context.filesDir, "xray/${p.uuid}.json").exists()) "json"
                            else "link"
                    ...
                    val src = when (p.protocol) {
                        ...
                        Protocol.XRAY -> File(context.filesDir, "xray/${p.uuid}.$ext")
                    ...
                    origNames["$dirName/$base"] = p.name
                    // OpenVPN keeps the user-entered credentials in its
                    // ProfileManager, not in the .ovpn file — capture them.
                    if (p.protocol == Protocol.OPENVPN) {
                        runCatching {
                            de.blinkt.openvpn.core.ProfileManager.get(context, p.uuid)
                        }.getOrNull()?.let { vp ->
                            val u = vp.mUsername.orEmpty()
                            val pw = vp.mPassword.orEmpty()
                            if (u.isNotBlank() || pw.isNotBlank()) {
                                creds["$dirName/$base"] = u to pw
                            }
                        }
                    }
                ...
                zip.write(
                    manifest(context, profiles, origNames, creds)
    ...
    private fun manifest(
        ...
        creds: Map<String, Pair<String, String>>,
    ): JSONObject {
        ...
        return JSONObject()
            ...
            .put("names", JSONObject().apply {
                for ((entry, orig) in origNames) {
                    if (entry.substringAfterLast('/') != orig) put(entry, orig)
                }
            })
            .put("credentials", JSONObject().apply {
                for ((entry, up) in creds) {
                    put(entry, JSONObject().put("u", up.first).put("p", up.second))
                }
            })
    }
    ...
    /**
     * Applies a manifest to freshly imported profiles. Imported profiles carry
     * the (possibly mangled) zip-entry base as their name, so the manifest key
     * is "$dir/${p.name}". In one pass we restore the original name, restore
     * OpenVPN credentials (they never lived in the .ovpn file), then match
     * favourites and the tile profile by the effective (restored) name.
     */
    fun applyManifest(context: Context, manifest: JSONObject, current: List<Profile>) {
        ...
        val names = manifest.optJSONObject("names")
        val creds = manifest.optJSONObject("credentials")
        for (p in current) {
            val dir = when (p.protocol) {
            ...
            val key = "$dir/${p.name}"
            val original = names?.optString(key, "")?.takeIf { it.isNotBlank() }
            if (original != null && original != p.name) {
                runCatching { VpnEngine.renameProfile(context, p.uuid, original) }
            }
            val effectiveName = original ?: p.name
            creds?.optJSONObject(key)?.let { c ->
                val u = c.optString("u", "")
                val pw = c.optString("p", "")
                if (u.isNotBlank() || pw.isNotBlank()) {
                    runCatching { VpnEngine.setCredentials(context, p.uuid, u, pw, persist = true) }
                }
            }
            if (effectiveName in favNames && !store.favourites().contains(p.uuid)) {
                store.toggleFavourite(p.uuid)
            }
            if (effectiveName == tileName) store.select(p)
        }
    }
