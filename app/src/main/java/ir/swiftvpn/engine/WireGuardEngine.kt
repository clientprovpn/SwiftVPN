...
class WireGuardEngine(context: Context) {
    ...
    /**
     * One-at-a-time gate for EVERY Backend.setState call (UP and DOWN alike).
     *
     * This exists because of a real crash: three rapid connect/disconnect
     * cycles ended with a native SIGSEGV (null dereference) that killed the
     * whole main process ~2.4s after the last DOWN. amneziawg-go's teardown
     * (device.Close, UAPI and socket shutdown) races with a concurrent
     * setState, and GoBackend itself keeps process-wide static state
     * (vpnService future, defaultTunnel) that is not safe for overlapping
     * calls either. Since a Go segfault is uncatchable from Kotlin, the only
     * defence is to never let transitions overlap — and to give the Go
     * runtime a moment to finish tearing down before the next UP.
     */
    private val stateMutex = kotlinx.coroutines.sync.Mutex()

    /** When the last DOWN completed; used to pace the next UP. */
    @Volatile
    ...
    suspend fun connect(uuid: String, store: WireGuardStore): String? =
        withContext(Dispatchers.IO) {
            ...
            try {
                ...
                val config = runCatching { org.amnezia.awg.config.Config.parse(configText.byteInputStream()) }
                    .getOrNull()
                    ?: return@withContext "This WireGuard profile could not be read"
                ...
                runCatching { disconnectBlockingLocked() }

                // A DOWN that just finished may still be unwinding inside the
                // Go runtime (device.Close, UAPI/socket shutdown). Bringing a
                // new device up on top of that teardown is the sequence that
                // produced the 2026-08-29 SIGSEGV, so the next UP is paced.
                val sinceDown = System.currentTimeMillis() - lastDownAt
                if (lastDownAt > 0L && sinceDown < DOWN_SETTLE_MS) {
                    delay(DOWN_SETTLE_MS - sinceDown)
                }
                ...
                try {
                    ...
                    // Offload truth on THIS device: 1 = legacy single-packet
                    // path, >1 = kernel GRO/GSO batching is live. Settles the
                    // "did the core swap change my throughput" question with a
                    // number instead of a guess.
                    val batch = runCatching {
                        (backend as? org.amnezia.awg.backend.GoBackend)?.getTunBatchSize() ?: -1
                    }.getOrDefault(-1)
                    if (batch > 0) {
                        onLog?.invoke(
                            if (batch == 1) "WireGuard: tun offload inactive (legacy single-packet path)"
                            else "WireGuard: tun offload ACTIVE (kernel batching, batchSize=$batch)",
                        )
                    }
                    null
    ...
    fun disconnectAsync() {
        ...
        active = null
        scope.launch {
            // Serialized with connect(): a DOWN launched from the UI must never
            // overlap a connect's UP that arrived a moment later.
            stateMutex.lock()
            try {
    ...
    /**
     * DOWN path for when the caller already holds [stateMutex] (connect's own
     * pre-teardown). Never call this without the lock held.
     */
    private fun disconnectBlockingLocked() {
        stopPolling()
        val tunnel = active ?: return
        active = null
        runCatching { backend.setState(tunnel, Tunnel.State.DOWN, null) }
     ...
     */
    private fun publishTunnelInfo(config: org.amnezia.awg.config.Config) {
    ...
    private companion object {
        ...
        /**
         * Minimum quiet time after a DOWN before the next UP. Gives the Go
         * runtime time to finish device teardown; see the stateMutex comment.
         */
        const val DOWN_SETTLE_MS = 400L
