                ...
                Scaffold(
                    ...
                    if (current == null) {
                        ProfileListScreen(
                            ...
                            onScanQr = { startScan() },
                            onImportClipboard = {
                                // Reads the clipboard directly — no paste box.
                                val cm = getSystemService(android.content.ClipboardManager::class.java)
                                val text = cm?.primaryClip
                                    ?.takeIf { it.itemCount > 0 }
                                    ?.getItemAt(0)?.coerceToText(this@MainActivity)?.toString()
                                if (text.isNullOrBlank()) {
                                    vm.reportMessage(getString(R.string.add_clipboard_empty))
                                } else {
                                    vm.importFromText(text)
                                }
                            },
                            onImportFile = {
                                // One picker for everything: .ovpn/.conf/share
                                // links, and zip archives are sniffed inside.
                                importLauncher.launch(arrayOf("*/*"))
                            },
                            onAddManual = { kind ->
                                when {
                                    kind == "warp" -> vm.createWarpProfile()
                                    kind == "ikev2" -> vm.createIkev2Profile()
                                    else -> vm.createXrayProfile(kind)
                                }
                            },
                            onOpenSubscriptions = { showSubscriptions = true },
                            onOpenLog = { showLog = true },
                            onBackup = vm::exportAll,
                            onTestDelay = vm::testAllLatency,
                            selected = selected,
