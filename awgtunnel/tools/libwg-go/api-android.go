...
type TunnelHandle struct {
	...
	uapi   net.Listener
	// SwiftVPN: tun batch size captured at startup, so the app can tell
	// whether kernel offloads (GRO/GSO, IFF_VNET_HDR) are active without
	// needing logcat access. 1 = legacy single-packet path.
	tunBatchSize int
}
...
func awgTurnOn(interfaceName string, tunFd int32, settings string) int32 {
	...
	// SwiftVPN: surface the offload state in the device log. GRO/GSO batching
	// only kicks in when the kernel tun fd advertises IFF_VNET_HDR, and the
	// batch size is the visible proof — 1 means the classic single-packet
	// path (what pre-2023 wireguard-go always used), anything larger means
	// the offload machinery is live on this device.
	// (interface assertion avoids the local variable shadowing the tun package)
	batchSize := 1
	if native, ok := interface{}(tun).(interface{ BatchSize() int }); ok {
		batchSize = native.BatchSize()
		logger.Verbosef("tun offload state: batchSize=%d (1 = legacy single-packet path)", native.BatchSize())
	}
	...
	tunnelHandles[i] = TunnelHandle{device: device, uapi: uapi, tunBatchSize: batchSize}
	return i
}

//export awgGetTunBatchSize
func awgGetTunBatchSize(tunnelHandle int32) int32 {
	if handle, ok := tunnelHandles[tunnelHandle]; ok {
		return int32(handle.tunBatchSize)
	}
	return -1
}
