...
public final class GoBackend implements Backend {
    ...
    /**
     * SwiftVPN: batch size of the live tun device, or -1 when no tunnel is up.
     * 1 = legacy single-packet path (no kernel GRO/GSO offload active).
     */
    public int getTunBatchSize() {
        if (currentTunnelHandle == -1)
            return -1;
        return awgGetTunBatchSize(currentTunnelHandle);
    }
