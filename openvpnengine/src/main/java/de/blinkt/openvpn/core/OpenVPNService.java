...
public class OpenVPNService extends VpnService implements StateListener, Callback, ByteCountListener, IOpenVPNServiceInternal {
    ...
    /*
     * LOCAL MODIFICATION (SwiftVPN): report speeds in BYTES per second.
     *
     * Upstream multiplied by 8 and formatted as bit/s / kbit/s / Mbit/s, so a
     * 14 MB/s transfer displayed as "112.2 Mbit/s". We want MB/s — the unit a
     * download manager shows — so the *8 is gone and speed now uses the same
     * 1024 base as volume.
     */
    public static String humanReadableByteCount(long bytes, boolean speed, Resources res) {
        ...
        if (speed)
            switch (exp) {
                case 0:
                    return res.getString(R.string.bytes_per_second, bytesUnit);
                case 1:
                    return res.getString(R.string.kbytes_per_second, bytesUnit);
                case 2:
                    return res.getString(R.string.mbytes_per_second, bytesUnit);
                default:
                    return res.getString(R.string.gbytes_per_second, bytesUnit);
            }
        else
            switch (exp) {
                case 0:
    ...
    /*
     * LOCAL MODIFICATION (SwiftVPN): pass the connection uptime as a fifth
     * argument so the status line can read
     *   "D/L: 11.73 MB/s  U/L: 104.09 kB/s  Uptime: 00:00:07"
     * rather than relying on the notification's corner chronometer.
     */
    @Override
    ...
    /**
     * LOCAL ADDITION (SwiftVPN): elapsed time since [since] as HH:MM:SS.
     * Returns 00:00:00 when the connect time is not known yet.
     */
    private static String formatUptime(long since) {
        if (since <= 0) return "00:00:00";
        long seconds = Math.max(0, (System.currentTimeMillis() - since) / 1000);
