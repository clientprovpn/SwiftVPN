...
static void set_options(char *logfile, jboolean ipv6)
{
	...
	/* SwiftVPN diagnostics: raise verbosity inside EAP tunnels so the inner
	 * identity and MSCHAPv2 exchange become visible when PEAP auth fails.
	 * Level 3 on IKE logs the raw tunneled EAP payloads. */
	lib->settings->set_int(lib->settings,
					"charon.filelog.android.ike", 3);
	lib->settings->set_int(lib->settings,
					"charon.filelog.android.tls", 2);
	lib->settings->set_int(lib->settings,
					"charon.filelog.android.cfg", 2);
