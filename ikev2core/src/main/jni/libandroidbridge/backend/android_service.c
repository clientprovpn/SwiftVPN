							...
							peer_cfg_t *peer_cfg, bool byod)
{
	...
	password = this->settings->get_str(this->settings, "connection.password",
									   NULL);
	/* SwiftVPN: optional EAP method preference ("mschapv2" / "peap" / "ttls").
	 * strongSwan's peer-side NAKs any other method the server proposes, so
	 * e.g. selecting MSCHAPv2 keeps Surfshark on the plain EAP-MSCHAPv2 path
	 * instead of entering its incompatible PEAP tunnel. */
	char *eap_type = this->settings->get_str(this->settings,
									   "connection.eap_type", NULL);
	if (eap_type)
	{
		if (strcaseeq(eap_type, "mschapv2"))
		{
			auth->add(auth, AUTH_RULE_EAP_TYPE, EAP_MSCHAPV2);
		}
		else if (strcaseeq(eap_type, "peap"))
		{
			auth->add(auth, AUTH_RULE_EAP_TYPE, EAP_PEAP);
		}
		else if (strcaseeq(eap_type, "ttls"))
		{
			auth->add(auth, AUTH_RULE_EAP_TYPE, EAP_TTLS);
		}
	}
