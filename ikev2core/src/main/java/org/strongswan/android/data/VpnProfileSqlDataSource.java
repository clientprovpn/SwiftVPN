...
public class VpnProfileSqlDataSource implements VpnProfileDataSource
{
	...
	{	/* SwiftVPN: own the database helper directly, no Application singleton */
	...
	private VpnProfile VpnProfileFromCursor(Cursor cursor)
	{
		...
		profile.setEapType(cursor.getString(cursor.getColumnIndexOrThrow(KEY_EAP_TYPE)));
		return profile;
	}
	...
	private ContentValues ContentValuesFromVpnProfile(VpnProfile profile)
	{
		...
		values.put(KEY_EAP_TYPE, profile.getEapType());
		return values;
	}
